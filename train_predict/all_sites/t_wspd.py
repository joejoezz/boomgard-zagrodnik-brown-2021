...
def fit_predict_one_station(X, y, train_size):
    ...
    if regressor == 'rf':
        regr = RandomForestRegressor(max_depth=None, random_state=0, verbose=2, n_estimators=n_estimators, n_jobs=-1)
    if regressor == 'lin':
...
df_combined = pd.concat((df_temp, df_wspd), axis=1)
...
for column in columns:
    ...
    X = df_simple.drop(columns=[column, column_wspd])
