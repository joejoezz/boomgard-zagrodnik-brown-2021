...
def fit_predict_one_station(X, y, train_size):
    ...
    # Save the values
    df_actual.to_pickle('/data/awn/impute/paper/data/train_predict/all_sites/{}_actual.p'.format(exp_name))
    df_predict.to_pickle('/data/awn/impute/paper/data/train_predict/all_sites/{}_predict.p'.format(exp_name))
    df_stats.to_csv('/data/awn/impute/paper/data/train_predict/all_sites/{}_stats.csv'.format(exp_name))
    return
...
for column in df_uwnd.columns:
    df_uwnd.rename(columns={column: '{}_uwnd'.format(column)}, inplace=True)
for column in df_vwnd.columns:
    df_vwnd.rename(columns={column: '{}_vwnd'.format(column)}, inplace=True)

# combine data here if necessary
df_combined = pd.concat((df_temp, df_uwnd, df_vwnd), axis=1)
...
for column in columns:
    ...
    column_uwnd = '{}_uwnd'.format(column)
    column_vwnd = '{}_vwnd'.format(column)
    X = df_simple.drop(columns=[column, column_uwnd, column_vwnd])
