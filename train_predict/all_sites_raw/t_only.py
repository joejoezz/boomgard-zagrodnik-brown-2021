...
def fit_predict_one_station(X, y, train_size):
    ...
    # Save the values
    df_actual.to_pickle('/data/awn/impute/paper/data/train_predict/all_sites_raw/{}_actual.p'.format(exp_name))
    df_predict.to_pickle('/data/awn/impute/paper/data/train_predict/all_sites_raw/{}_predict.p'.format(exp_name))
    df_stats.to_csv('/data/awn/impute/paper/data/train_predict/all_sites_raw/{}_stats.csv'.format(exp_name))
    return


# load QCed temp to validate model
df_temp_c = pd.read_pickle('/data/awn/impute/awn-impute/data/clean/temp_clean.p')
df_temp_c.index = pd.to_datetime(df_temp_c.index)


#df_temp.dropna(how='all', inplace=True)
#df_temp = pd.read_pickle('/data/awn/impute/awn-impute/data/clean/temp_clean.p')
#df_temp.index = pd.to_datetime(df_temp.index)
t0_dir = '/data/awn/impute/awn-impute/data/'
df_temp = pd.read_pickle('{}full_t_raw.p'.format(t0_dir))
