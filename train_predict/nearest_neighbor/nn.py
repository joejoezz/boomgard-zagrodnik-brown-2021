...
def fit_predict_one_station(X, y, train_size):
    ...
    df_actual[column] = y
    df_predict[column] =  X_test#-X_test[X.columns[0]]
...
drop_thres = len(df_temp)*0.3
na_count = df_temp.isna().sum()
...
df_stats = pd.DataFrame(index=columns, columns=['rmse','mean', 'std', 'max', 'pct95', 'pct99', 'pct999', 'mean_distance', 'time'])
